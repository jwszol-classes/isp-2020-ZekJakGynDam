...
class GeocodeData:
    ...
    def load_countries(self, country_filename):
        ...
        """
        self.countries = {}
        for code, name in csv.reader(open(country_filename, encoding="utf-8")):
    ...
    def extract(self, local_filename):
        ...
        if os.path.exists(local_filename):
            ...
            if not os.path.exists(GEOCODE_FILENAME):
                ...
                open(GEOCODE_FILENAME, 'wb', encoding="utf-8").write(z.read(GEOCODE_FILENAME))

            # extract coordinates into more compact CSV for faster loading
            writer = csv.writer(open(local_filename, 'w', encoding="utf-8"))
            rows = []
            for row in csv.reader(open(GEOCODE_FILENAME, encoding="utf-8"), delimiter='\t'):
